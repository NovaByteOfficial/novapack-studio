  ...
  async function handleAPICall(type, payload, requestId, app, webview, sandbox) {
    ...
    try {
      ...
      if (type === 'nova:net:websocket') {
        ...
        const netPerm = isInternal ? 'net:internal' : 'net:external';
        if (!AppPermissionManager.isGranted(netPerm, app.id)) {
          return respondError(webview, type, requestId, 'PERMISSION_DENIED', `${netPerm} permission required for WebSocket`);
        }
        let ws;
        try {
          ws = protocols?.length
      ...
      if (type === 'nova:net:beacon') {
        ...
        const netPerm = isInternal ? 'net:internal' : 'net:external';
        if (!AppPermissionManager.isGranted(netPerm, app.id)) {
          return respondError(webview, type, requestId, 'PERMISSION_DENIED', `${netPerm} permission required for beacon`);
        }
        try {
          const res = await fetch(url, {
            method: 'POST',
          ...
          return respond(webview, type, requestId, { success: ok, status: res.status });
        } catch (e) {
          return respondError(webview, type, requestId, 'NETWORK_ERROR', e.message);
        }
      }
...
(function(){
  var _novaReady = false;
  var _novaInitPromise = new Promise(function(resolve, reject) {
    function readyHandler(e) {
      if (!e.data || e.data.type !== 'nova:ready') return;
      window.removeEventListener('message', readyHandler);
      _novaReady = true;
      resolve();
    }
    window.addEventListener('message', readyHandler);
    window.parent.postMessage({ type: 'nova:ping' }, '*');
    setTimeout(function() { resolve(); }, 2000);
  });

  function _ipc(type, payload) {
    return _novaInitPromise.then(function() {
  ...
  window.nova = {
    onEvent: function(eventName, handler) {
      if (typeof eventName !== 'string' || typeof handler !== 'function') {
        return {
          then: function(onF, onR) { return Promise.reject(new TypeError('onEvent requires a string event name and function handler')).then(onF, onR); },
          unsubscribe: function() {}
        };
      }
      var eventHandlers = window.nova._eventHandlers;
      var unsub = function() {
        return window.nova.offEvent(eventName);
      };
      if (eventHandlers.has(eventName)) {
        eventHandlers.set(eventName, handler);
        var p = Promise.resolve({ subscribed: eventName, replaced: true });
        p.unsubscribe = unsub;
        return p;
      }
      var p = _ipc('nova:events:subscribe', { event: eventName }).then(function(result) {
        eventHandlers.set(eventName, handler);
        return result;
      });
      p.unsubscribe = unsub;
    ...
    offEvent: function(eventName) {
      var eventHandlers = window.nova._eventHandlers;
      if (!eventHandlers.has(eventName)) return Promise.resolve({ unsubscribed: eventName, wasSubscribed: false });
      var handler = eventHandlers.get(eventName);
      return _ipc('nova:events:unsubscribe', { event: eventName }).then(function(result) {
    ...
    _dispatchEvent: function(eventName, data) {
      var handler = window.nova._eventHandlers.get(eventName);
  ...
  window.addEventListener('message', function(e) {
    ...
    if (e.data.type === 'nova:events:event:response' && !e.data.error && e.data.result) {
      var payload = e.data.result;
      if (payload && payload.event !== undefined) {
        window.nova._dispatchEvent(payload.event, payload.data);
      }
    }
  });
  ...
  if (typeof navigator !== 'undefined') {
    ...
    navigator.sendBeacon = function(url, data) {
      return _ipc('nova:net:beacon', { url: url, data: data }).catch(function() { return false; });
    };
  }
})()
\x3C/script>`;
